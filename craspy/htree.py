...
class HRTree():
    def __init__(self,table,synth):
        ...
        if table.meta['KERNEL'] != 'METABUBBLE':
            ...
            return
        self.rep, self.delta, self.noise, self.kernel = HRep.toTuple(table)
        self.synth = synth
    ...
    def synthesize(self,clumps):
        ...
        if not isinstance(clumps,list):
            clumps=[clumps]
        for i in clumps:
            newSyn = synthesize_bubbles(newSyn, self.clumps[i], self.kernel, self.noise, self.delta)
        return Data(newSyn * self.scale - self.shift, wcs=self.synth.wcs, unit=self.synth.unit, meta=self.synth.meta,mask=self.synth.mask)
