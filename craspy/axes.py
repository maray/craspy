...
from .index import _index_features, _fix_limits
...
from .formatting import _unitize, _world_table_creator
...
def features(data, wcs=None, region=None):
    ...
    if wcs is None:
        log.error("A world coordinate system (WCS) is needed")
        return None
    ii = _index_features(data, lower, upper)
...
def fov(data, center, window, wcs=None):
    """
    ...
    lower = _fix_limits(data, lower[0][::-1])
    upper = _fix_limits(data, upper[0][::-1])
    return lower, upper
