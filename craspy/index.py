...
def _slab(data, lower=None, upper=None):
    """
    ...
    if upper is None:
        upper = data.shape
    lower = _fix_limits(data, lower)
    upper = _fix_limits(data, upper)
    m_slab = []
...
def _matching_slabs(data, flux, lower, upper):
    """
...
def _index_features(data, lower=None, upper=None):
